<link rel="stylesheet" href="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/css/mapStyle.css" />

<link rel="stylesheet" href="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/css/leaflet.css" />
<link rel="stylesheet" href="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/css/L.Control.HtmlLegend.css" />
<script src="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/js/leaflet.js"></script>
<!--
...
<script src="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/js/L.TileLayer.BetterWMS.js"></script>
<script src="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/js/proj4.js"></script>
<script src="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/js/leaflet.sld.js"></script>
<script src="<?=get_home_url('url') ?>/wp-content/plugins/wp-geolib/js/L.Control.HtmlLegend.js"></script>
...
$url = get_home_url('url').'/wp-content/plugins/wp-geolib/files'.$filepath;
...
<!-- ヘッダタイトルの表示 -->
<div id="map_title">
	<h2>【<?= $libraryname ?>】 <?=$title ?>
	<input value="前に戻る" onclick="history.back();" type="button"></h2>
	<p><?= $abstract ?></p>
</div>
...
<!-- 凡例パネル表示 -->
    ...
    foreach ($maps as $map){
        if($map->maptype=='Scenario'){
            ...
            ?>
	        <div><input type="checkbox" name="<?=$mapId?>" onclick='setLayerVisible(this.checked,"<?=$mapId?>","lg");'><?=$maptitle?></div>
    ...
    foreach ($maps as $map){
        if($map->maptype=='Subject'){
            ...
            // geoTiff
                ...
                ?>
	        	<div><input type="checkbox" name="<?=$mapId?>" onclick='setLayerVisible(this.checked,"<?=$mapId?>","l");'><?=$maptitle?></div>
